...
/**
 * Description of B2WL_Init
 *
 * @author andrey
 */
if (!class_exists('B2WL_Init')) {
    ...
    class B2WL_Init {
        ...
        static public function init_classes($classpath, $default_load_action, $params = array()) {
            $this_class = B2WL_Init::getInstance();
                    ...
                    if (is_file($f)) {
                        ...
                        if ($file_info["extension"] == "php") {
                            ...
                            if (isset($file_data['include_action']) && $file_data['include_action']) {
                                $include_action = (!is_null(filter_var($file_data['include_action'], FILTER_VALIDATE_BOOLEAN, FILTER_NULL_ON_FAILURE))) ? (filter_var($file_data['include_action'], FILTER_VALIDATE_BOOLEAN)?self::DEFAULT_INCLIDE_ACTION:"b2wl_fake_action") : $file_data['include_action'];
