...
if (!function_exists('b2w_global_template_redirect')) {

    function b2w_global_template_redirect()
    {
        do_action('b2w_template_redirect');

        do_action('b2w_after_template_redirect');
    }
...
if (!class_exists('B2WL_JSON_API')) {
    ...
    class B2WL_JSON_API
    {
        ...
        public function __construct($root_menu_slug)
        {
            $this->root_menu_slug = $root_menu_slug;
            $this->query = new B2WL_JSON_API_Query();
            $this->response = new B2WL_JSON_API_Response();
            add_action('b2w_template_redirect', array(&$this, 'template_redirect'));

            add_action('b2w_after_template_redirect', array(&$this, 'after_template_redirect'));

            if (!has_action('template_redirect', 'b2w_global_template_redirect')) {
                add_action('template_redirect', 'b2w_global_template_redirect');
            }

            // disable menu
            if (defined('B2WL_DEBUG_PAGE') && B2WL_DEBUG_PAGE) {
                add_action('admin_menu', array(&$this, 'admin_menu'));
            }

            add_action('update_option_b2wl_json_api_base', array(&$this, 'flush_rewrite_rules'));
            add_action('pre_update_option_b2wl_json_api_controllers', array(&$this, 'update_controllers'));
        }
        ...
        public function template_redirect()
        {
            ...
            $enabled_controllers = explode(',', b2wl_get_setting('json_api_controllers'));
            ...
            if ($controller) {
                ...
                if ($method) {
                    ...
                    do_action("b2wl_json_api", $controller, $method);
                    do_action("b2wl_json_api-{$controller}-$method");
        ...
        {
            add_submenu_page($this->root_menu_slug, 'JSON API Settings', 'JSON API', 'manage_options', 'b2w-json-api', array(&$this, 'admin_options'));
        }

        public function admin_options()
        {
            if (!current_user_can('manage_options')) {
                wp_die(__('You do not have sufficient permissions to access this page.', 'bng2woo-lite'));
            }
            ...
            if (!empty($_REQUEST['_wpnonce']) && wp_verify_nonce($_REQUEST['_wpnonce'], "update-options")) {
                if ((!empty($_REQUEST['action']) || !empty($_REQUEST['action2'])) &&
                    ...
                    b2wl_set_setting('json_api_controllers', implode(',', $active_controllers));
                }
                if (isset($_REQUEST['b2wl_json_api_base'])) {
                    b2wl_set_setting('json_api_base', $_REQUEST['b2wl_json_api_base']);
                }
            }
            ?>
            <div class="wrap">
                ...
                <form action="admin.php?page=b2w-json-api" method="post">
...
foreach ($available_controllers as $controller) {
                ...
                <?php
if ($active) {
                    echo '<a href="' . wp_nonce_url('admin.php?page=b2w-json-api&amp;action=deactivate&amp;controller=' . $controller, 'update-options') . '" title="' . __('Deactivate this controller', 'bng2woo-lite') . '" class="edit">' . __('Deactivate') . '</a>';
                } else if (!$error) {
                    echo '<a href="' . wp_nonce_url('admin.php?page=b2w-json-api&amp;action=activate&amp;controller=' . $controller, 'update-options') . '" title="' . __('Activate this controller', 'bng2woo-lite') . '" class="edit">' . __('Activate') . '</a>';
                }
...
foreach ($info['methods'] as $method) {
            ...
            <?php $this->print_controller_actions('action2');?>
                    ...
                    <table class="form-table">
                        <tr valign="top">
                            ...
                            <td><code><?php bloginfo('url');?>/</code><input type="text" name="b2wl_json_api_base" value="<?php echo b2wl_get_setting('json_api_base'); ?>" size="15" /></td>
                        </tr>
                    </table>
            ...
            <?php }?>
                    <p class="submit">
                        <input type="submit" class="button-primary" value="<?php _e('Save changes', 'bng2woo-lite')?>" />
                    </p>
                </form>
            </div>
                        <?php
        ...
        public function get_method_url($controller, $method, $options = '')
        {
            ...
            if (!empty($base) && !empty($permalink_structure)) {
                ...
                return "$url?b2w-json=$method&$args";
            }
        }
        ...
        {
            $controllers = array();
            $this->check_directory_for_controllers(B2WL_JSON_API_DIR . "/controllers", $controllers);
            $this->check_directory_for_controllers(get_stylesheet_directory(), $controllers);
            $controllers = apply_filters('b2wl_json_api_controllers', $controllers);
            return array_map('strtolower', $controllers);
        }

        public function get_enabled_controllers($default = 'core,auth')
        {
            return explode(',', b2wl_get_setting('json_api_controllers', $default));
        }
        ...
        public function check_directory_for_controllers($dir, &$controllers)
        {
            ...
            while ($file = readdir($dh)) {
                if (preg_match('/(.+)\.php$/i', $file, $matches)) {
                    ...
                    if (preg_match("/class\s+B2WL_JSON_API_{$matches[1]}_Controller/i", $src)) {
        ...
        {
            if (defined('B2WL_JSON_API_CONTROLLERS')) {
                $default = B2WL_JSON_API_CONTROLLERS;
            } else {
                $default = 'core,auth';
            }
            $active_controllers = explode(',', b2wl_get_setting('json_api_controllers', $default));
        ...
        {
            return "b2wl_json_api_{$controller}_controller";
        }

        public function controller_path($controller)
        {
            $json_api_path = B2WL_JSON_API_DIR . "/controllers/$controller.php";
        ...
        public function get_nonce_id($controller, $method)
        {
            ...
            return "b2wl_json_api-$controller-$method";
        }
