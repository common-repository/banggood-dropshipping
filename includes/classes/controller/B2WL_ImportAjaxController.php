...
/**
 * Description of B2WL_ImportAjaxController
 *
 * @author Andrey
 *
 ...
 * @ajax: true
 */
if (!class_exists('B2WL_ImportAjaxController')) {

    class B2WL_ImportAjaxController
    {
        public function __construct()
        {
            ...
            add_action('wp_ajax_b2wl_add_to_import', array($this, 'ajax_add_to_import'));
            add_action('wp_ajax_b2wl_remove_from_import', array($this, 'ajax_remove_from_import'));
                    ...
                    if ($product) {
                        try {
                            ...
                            if ($result['state'] !== 'error') {
                                // write firt step log
                                b2wl_info_log("IMPORT[time: " . (microtime(true) - $ts) . ", id:" . $result['product_id'] . ", extId: " . $_POST['id'] . ", step: init]");

                                // move product to pricessing list
                                $product_import_model->move_to_processing($_POST['id']);

                                // process all other steps
                                $product_queue = B2WL_ImportProcess::create_new_queue($result['product_id'], $_POST['id'], $steps, false);
                                if (b2wl_get_setting('load_review')) {
                                    B2WL_ImportProcess::create_new_queue($result['product_id'], $_POST['id'], array('reviews'), false);
                                }
                        ...
                        $result = B2WL_ResultBuilder::buildError("Product " . $_POST['id'] . " not find.");
                    }
                } else {
                ...
                try {
                    if (isset($_POST['id']) && $_POST['id']) {
                        ...
                        if ($product) {
                            ...
                            if (!b2wl_get_setting('allow_product_duplication') && $import_wc_product_id) {
                            ...
                            /*   if ($result['state'] !== 'error' && b2wl_get_setting('load_review')) {
        ...
        {
            $product_import_model = new B2WL_ProductImport();
            echo json_encode(B2WL_ResultBuilder::buildOk(array('ids' => $product_import_model->get_product_id_list())));
            wp_die();
        }
        ...
        public function ajax_override_order_variations()
        {
            ...
            if ($result['state'] != 'error') {
                $override_model = new B2WL_Override();
        ...
        public function ajax_add_to_import()
        {
            if (isset($_POST['id'])) {
                $product = array();
                $products = b2wl_get_transient('b2wl_search_result');

                $product_import_model = new B2WL_ProductImport();
                $loader = new B2WL_Banggood();
                ...
                global $wpdb;
                $post_id = $wpdb->get_var($wpdb->prepare("SELECT post_id FROM $wpdb->postmeta WHERE meta_key='_b2w_external_id' AND meta_value='%s' LIMIT 1", $_POST['id']));
                if (b2wl_get_setting('allow_product_duplication') || !$post_id) {
                    ...
                    if ($res['state'] !== 'error') {
                        ...
                        if ($product) {
                            ...
                            echo json_encode(B2WL_ResultBuilder::buildError("Product not found in serach result"));
                        }
                    } else {
                        echo json_encode($res);
                    }
                } else {
                    echo json_encode(B2WL_ResultBuilder::buildError("Product already imported."));
                }
            } else {
                echo json_encode(B2WL_ResultBuilder::buildError("add_to_import: waiting for ID..."));
            }
            wp_die();
        }
        ...
        public function ajax_remove_from_import()
        {
            if (isset($_POST['id'])) {
                ...
                echo json_encode(B2WL_ResultBuilder::buildError("remove_from_import: waiting for ID..."));
            }
            wp_die();
        }
