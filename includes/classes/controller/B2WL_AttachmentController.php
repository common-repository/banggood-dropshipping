...
/**
 * Description of B2WL_AttachmentController
 *
 * @author Andrey
 * 
 * @autoload: b2wl_init
 */
if (!class_exists('B2WL_AttachmentController')) {

    class B2WL_AttachmentController {
        ...
        function get_attachment_url($url, $id) {
            // if not b2w attached return to default function
            if (!get_post_meta($id, '_wp_b2w_attached_file', true)) {
                return $url;
            }
        ...
        function calculate_image_srcset($sources, $size_array, $image_src, $image_meta, $attachment_id) {
            // if not b2w attached return to default function
            if (!$sources || !get_post_meta($attachment_id, '_wp_b2w_attached_file', true)) {
                return $sources;
            }
