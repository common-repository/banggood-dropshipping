<?php
/**
 * Description of B2WL_ProductServiceController
 *
 * @author Andrey
 *
 * @autoload: b2wl_admin_init
 */
if (!class_exists('B2WL_ProductServiceController')) {

    class B2WL_ProductServiceController
    {
        ...
        public function delete_post_action($post_id)
        {
            ...
            if (!b2wl_check_defined('B2WL_DO_NOT_USE_TRANSACTION') &&
                !isset($GLOBALS['b2wl_delete_post_register_shutdown']) &&
                in_array($post_type, array('product', 'product_variation'))
            ) {
                $GLOBALS['b2wl_delete_post_register_shutdown'] = 1;
                ...
                register_shutdown_function(function () {
                    unset($GLOBALS['b2wl_delete_post_register_shutdown']);
            ...
            B2WL_Utils::delete_post_images($post_id);

            //second, mark variation as skip
            if ($post_type == 'product_variation' && !isset($GLOBALS['b2wl_autodelete_variaton_lock'])) {
                $variation = new WC_Product_Variation($post_id);
                $parent_id = $variation->get_parent_id();
                $b2wl_skip_meta = get_post_meta($parent_id, "_b2w_skip_meta", true);
                ...
                if (!in_array($external_variation_id, $b2wl_skip_meta['skip_vars'])) {
                    $b2wl_skip_meta['skip_vars'][] = $external_variation_id;
                }
                update_post_meta($parent_id, "_b2w_skip_meta", $b2wl_skip_meta);

                $this->update_banggood_sku_props($parent_id, $post_id);
            }
        }
        ...
        public function save_product_variation($variation_id, $i)
        {
            ...
            if (in_array($variation->get_status(), array('publish', false))) {
                $b2wl_skip_meta['skip_vars'] = array_diff($b2wl_skip_meta['skip_vars'], array($external_variation_id));
            } else {
                if (!in_array($external_variation_id, $b2wl_skip_meta['skip_vars'])) {
                    $b2wl_skip_meta['skip_vars'][] = $external_variation_id;
                }
            ...
            $this->update_banggood_sku_props($parent_id);
        }

        private function update_banggood_sku_props($product_id, $skip_ids = array())
        {
            ...
            if ($product) {
                ...
                foreach ($var_ids as $var_id) {
                    if (!in_array($var_id, $skip_ids)) {
                        $banggood_sku_props = get_post_meta($var_id, "_banggood_sku_props", true);
                        if ($banggood_sku_props) {
                            update_post_meta($product_id, '_banggood_sku_props', $banggood_sku_props);
                            break;
                        }
                    }
                }
        ...
        public function woocommerce_save_attributes()
        {
            ...
            if ($original_variations_attributes) {
                ...
                $product_type = !empty($_POST['product_type']) ? sanitize_key($_POST['product_type']) : 'simple';
                ...
                update_post_meta($product_id, '_b2w_deleted_variations_attributes', $original_variations_attributes);
            }
        }
    }
