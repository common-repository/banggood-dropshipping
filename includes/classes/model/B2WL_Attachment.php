...
/**
 * Description of B2WL_Attachment
 *
 * @author Andrey
 */
if (!class_exists('B2WL_Attachment')) {

    class B2WL_Attachment {

        private $utils;
        private $use_external_image_urls = false;

        public function __construct($use_external = 'cfg') {
            $this->utils = new B2WL_Utils();
            if ('external' === $use_external || 'local' === $use_external) {
                $this->use_external_image_urls = ('external' === $use_external);
            } else {
                $this->use_external_image_urls = b2wl_get_setting('use_external_image_urls');
            }
        }
        ...
        public function create_attachment($post_id, $image_path, $params = array()) {
            ...
            $image_path = B2WL_Utils::clear_image_url($image_path);
            ...
            if (!empty($edit_images[$image_id])) {
                ...
                wp_update_post(array(
                    ...
                    'meta_input' => array('_b2w_external_image_url' => $edit_images[$image_id]['external_image_url'])
                ));
            ...
            if ($check_duplicate) {
                if($use_external_image_urls){
                    $old_attachment_id = $wpdb->get_var($wpdb->prepare("SELECT ID FROM $wpdb->posts p INNER JOIN $wpdb->postmeta pm on (p.ID = pm.post_id) WHERE p.post_parent=%d and pm.meta_key='_b2w_external_image_url' AND pm.meta_value=%s LIMIT 1", $post_id, $image_path));
                }else{
                    $old_attachment_id = $wpdb->get_var($wpdb->prepare("SELECT ID FROM $wpdb->posts p INNER JOIN $wpdb->postmeta pm on (p.ID = pm.post_id) LEFT JOIN $wpdb->postmeta pm1 ON (p.ID = pm1.post_id and pm1.meta_key='_wp_b2w_attached_file' and pm1.meta_value='1') WHERE p.post_parent=%d and pm.meta_key='_b2w_external_image_url' AND pm.meta_value=%s AND pm1.meta_id is null LIMIT 1", $post_id, $image_path));
                }
            ...
            if ($use_external_image_urls) {
                ...
                update_post_meta($attach_id, '_b2w_external_image_url', $image_path);
        ...
        public function create_attachment_from_data($post_id, $file_data, $params = array()) {
            ...
            $ext = B2WL_Attachment::mime2ext($matches[1]);
            if (!$ext) {
        ...
        private function set_attachment_metadata($attach_id, $image_url) {
            update_post_meta($attach_id, '_wp_b2w_attached_file', 1);

            $pi = pathinfo($image_url);

            // original size 1051x1051
            // /images/->/thumb/gallery 235x235
            // /images/->/thumb/grid 163x163
            // /images/->/thumb/home 85x85
            // /images/->/thumb/list_grid 120x120
            // /images/->/thumb/other_items/ 50x50
            // /images/->/thumb/view 361x361


            $image_sizes = array();
            if (!empty($pi['extension'])) {
                $image_sizes['thumbnail'] = array('url' => str_replace("/images/", "/thumb/other_items/", $image_url), 'width' => 50, 'height' => 50);
                $image_sizes['small1'] = array('url' => str_replace("/images/", "/thumb/list_grid/", $image_url), 'width' => 100, 'height' => 100);
                $image_sizes['small2'] = array('url' => str_replace("/images/", "/thumb/gallery/", $image_url), 'width' => 200, 'height' => 200);
                $image_sizes['medium'] = array('url' => str_replace("/images/", "/thumb/view/", $image_url), 'width' => 350, 'height' => 350);
                // $image_sizes['medium_large'] = array('url' => $image_url, 'width' => 640, 'height' => 640);
            }
            ...
            foreach ($wp_sizes as $size => $props) {
                $found_size = $this->_choose_image_size_from_banggood($props, $image_sizes);
        ...
        private function _choose_image_size_from_banggood($size, $image_sizes = array()) {
        ...
        public static function find_products_with_external_images() {
            global $wpdb;
            $result_ids = array();
            $tmp_product_ids = $wpdb->get_results("select distinct if(p.post_parent = 0, p.id, p.post_parent) as id from $wpdb->posts p, (select distinct p1.post_parent as id from $wpdb->posts p1 LEFT join $wpdb->postmeta pm1 on(p1.id = pm1.post_id) where p1.post_type = 'attachment' and pm1.meta_key='_wp_b2w_attached_file' and pm1.meta_value='1') pp WHERE p.ID = pp.id", ARRAY_N);
        ...
        public static function calc_total_external_images() {
            global $wpdb;
            $cnt = $wpdb->get_var("select count(id) from (select distinct p1.id as id from $wpdb->posts p1 LEFT join $wpdb->postmeta pm1 on(p1.id = pm1.post_id) where p1.post_type = 'attachment' and pm1.meta_key='_wp_b2w_attached_file' and pm1.meta_value='1') as pp");
        ...
        public static function find_external_images($page_size = 1000) {
            global $wpdb;
            $result_ids = array();
            $tmp_product_ids = $wpdb->get_results("select distinct p1.id as id from $wpdb->posts p1 LEFT join $wpdb->postmeta pm1 on(p1.id = pm1.post_id) where p1.post_type = 'attachment' and pm1.meta_key='_wp_b2w_attached_file' and pm1.meta_value='1' LIMIT $page_size", ARRAY_N);
            foreach ($tmp_product_ids as $row) {
                $result_ids[] = $row[0];
            }
        ...
        public function load_external_image($post_id) {
            ...
            if ($post_id) {
                ...
                if ($post->post_type === 'attachment') {
                    $tmp = get_post_meta($post_id, '_wp_b2w_attached_file', true);
                    if ($tmp && intval($tmp) === 1) {
                        ...
                        if ($new_image_id) {
                            ...
                            B2WL_Utils::delete_attachment($post_id, true);
