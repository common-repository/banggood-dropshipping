...
/**
 * Description of B2WL_Woocommerce
 *
 * @author andrey
 */
if (!class_exists('B2WL_Woocommerce')) {

    class B2WL_Woocommerce
    {
        ...
        {
            $this->attachment_model = new B2WL_Attachment();
            $this->helper = new B2WL_Helper();
            $this->product_change_model = new B2WL_ProductChange();
        }
        ...
        private function get_woocommerce_manage_stock()
        {
            $woocommerce_manage_stock = get_option('woocommerce_manage_stock', 'no');
            return $woocommerce_manage_stock;
        }
        ...
        public function build_steps($product)
        {
            ...
            $images_to_preload = B2WL_Utils::get_all_images_from_product($product, true, !$override_product || $override_images, !$override_product || $override_title_description);
        ...
        {
            $product_type = $product_type ? $product_type : ((isset($product['product_type']) && $product['product_type']) ? $product['product_type'] : b2wl_get_setting('default_product_type', 'simple'));
            return !b2wl_check_defined('B2WL_DO_NOT_IMPORT_VARIATIONS') &&
            ...
            if (isset($product['thumb_id'])) {
                foreach ($tmp_all_images as $img_id => $img) {
                    if ($img_id === $product['thumb_id'] && !in_array($img_id, $product['skip_images'])) {
                        $thumb_url = B2WL_Utils::clear_url($img['image']);
                        break;
                    }
                }
            }
        ...
        {
            if (!B2WL_Woocommerce::is_woocommerce_installed()) {
                return B2WL_ResultBuilder::buildError("Woocommerce is not installed");
            }

            global $wpdb;

            $woocommerce_manage_stock = $this->get_woocommerce_manage_stock();
            ...
            $product_type = (isset($product['product_type']) && $product['product_type']) ? $product['product_type'] : b2wl_get_setting('default_product_type', 'simple');
            $product_status = (isset($product['product_status']) && $product['product_status']) ? $product['product_status'] : b2wl_get_setting('default_product_status', 'publish');
            ...
            if ($step === false || $step === 'init') {
                do_action('b2wl_woocommerce_before_add_product', $product, $params);
                ...
                $post = array(
                    ...
                    'meta_input' => array('_stock_status' => 'instock',
                        ...
                        '_b2w_external_id' => $product['id'],
                        '_b2w_import_id' => $product['import_id'],
                        '_b2w_product_url' => $product['affiliate_url'],
                        '_b2w_original_product_url' => $product['url'],
                        '_b2w_seller_url' => (!empty($product['seller_url']) ? $product['seller_url'] : ''),
                        '_b2w_seller_name' => (!empty($product['seller_name']) ? $product['seller_name'] : ''),
                        '_b2w_warehouse' => $product['warehouse'], // this field contains a default warehouse,in case if this is a simple product
                        '_b2w_warehouse_list' => $product['warehouse_list'],
                        '_b2w_last_update' => time(),
                        '_b2w_skip_meta' => array('skip_vars' => $product['skip_vars'], 'skip_images' => $product['skip_images']),
                        '_b2w_disable_sync' => 0,
                        '_b2w_disable_var_price_change' => isset($product['disable_var_price_change']) && $product['disable_var_price_change'] ? 1 : 0,
                        '_b2w_disable_var_quantity_change' => isset($product['disable_var_quantity_change']) && $product['disable_var_quantity_change'] ? 1 : 0,
                        '_b2w_disable_add_new_variants' => isset($product['disable_add_new_variants']) && $product['disable_add_new_variants'] ? 1 : 0,
                        '_b2w_orders_count' => (!empty($product['ordersCount']) ? intval($product['ordersCount']) : 0),
                        '_b2w_video' => !empty($product['video']) ? $product['video'] : '',
                        '_b2w_import_lang' => !empty($product['import_lang']) ? $product['import_lang'] : B2WL_BanggoodLocalizator::getInstance()->language,
                    ),
                );
                ...
                if ($override_product && $this->is_product_exist($product_id)) {
                    ...
                    if ($override_images) {
                        ...
                        foreach ($attachments as $attachment_id) {
                            B2WL_Utils::delete_attachment($attachment_id, true);
                        }
                    ...
                    foreach ($variations as $var_id) {
                        ...
                        $var->delete(true);
                        B2WL_Utils::delete_post_images($var_id);
                    }

                    // Delete variations attributes
                    delete_post_meta($product_id, "_b2w_original_variations_attributes");
                    ...
                    delete_post_meta($product_id, "_b2w_skip_meta");
                ...
                if ($first_ava_var) {
                    delete_post_meta($product_id, "_b2w_outofstock");
                } else {
                    update_post_meta($product_id, "_b2w_outofstock", true);
                }

                // set default shipping country from
                if ($first_ava_var && !empty($first_ava_var['warehouse'])) {
                    update_post_meta($product_id, '_b2w_warehouse', $first_ava_var['warehouse']);
                }

                // save shipping meta data
                $shipping_meta = new B2WL_ProductShippingMeta($product_id);
                ...
                $default_shipping_class = b2wl_get_setting('default_shipping_class');
                if ($default_shipping_class) {
                    wp_set_object_terms($product_id, intval($default_shipping_class), 'product_shipping_class');
                }

                if ($step !== false) {
                    return $result = B2WL_ResultBuilder::buildOk(array('product_id' => $product_id, 'step' => $step));
                }

            }

            if ($step !== false && !$this->is_product_exist($product_id)) {
                return B2WL_ResultBuilder::buildError("Error! Processing processing product($product_id) not found");
            }

            if (substr($step, 0, strlen('preload_images')) === 'preload_images') {
                $images_to_preload = B2WL_Utils::get_all_images_from_product($product, true);
                $cnt = 0;
                foreach ($images_to_preload as $img_id => $image) {
                    $cnt++;
                ...
                return B2WL_ResultBuilder::buildOk(array('product_id' => $product_id, 'step' => $step));
            }
            ...
            if ($step === false || substr($step, 0, strlen('variations')) === 'variations') {
                if ($this->need_import_variations($product, $product_type)) {
                    ...
                    $banggood_sku_props_id_arr = array();
                    foreach ($first_ava_var['attributes'] as $cur_var_attr) {
                        foreach ($product['sku_products']['attributes'] as $attr) {
                            if (isset($attr['value'][$cur_var_attr])) {
                                $banggood_sku_props_id_arr[] = isset($attr['value'][$cur_var_attr]['original_id']) ? $attr['value'][$cur_var_attr]['original_id'] : $attr['value'][$cur_var_attr]['id'];
                                break;
                            }
                        }
                    }
                    $banggood_sku_props_id = $banggood_sku_props_id_arr ? implode(";", $banggood_sku_props_id_arr) : "";
                    if ($banggood_sku_props_id) {
                        update_post_meta($product_id, '_banggood_sku_props', $banggood_sku_props_id);
                    }
                }
                if ($step !== false) {
                    return B2WL_ResultBuilder::buildOk(array('product_id' => $product_id, 'step' => $step));
            ...
            if ($step === false || $step === 'finishing') {
                ...
                do_action('b2wl_add_product', $product_id);

                B2WL_Utils::update_post_terms_count($product_id);
            }

            return apply_filters('b2wl_woocommerce_after_add_product', B2WL_ResultBuilder::buildOk(array('product_id' => $product_id)), $product_id, $product, $params);
        }

        public function upd_product($product_id, $product, $params = array())
        {
            do_action('b2wl_woocommerce_upd_product', $product_id, $product, $params);
            ...
            if (!empty($product['affiliate_url']) && !b2wl_check_defined('B2WL_DISABLE_UPDATE_AFFILIATE_URL')) {
                update_post_meta($product_id, '_product_url', $product['affiliate_url']);
                update_post_meta($product_id, '_b2w_product_url', $product['affiliate_url']);
            }

            if (!empty($product['url'])) {
                update_post_meta($product_id, '_b2w_original_product_url', $product['url']);
            }

            if (!empty($product['ordersCount'])) {
                update_post_meta($product_id, '_b2w_orders_count', intval($product['ordersCount']));
            }

            if (!empty($product['video'])) {
                update_post_meta($product_id, '_b2w_video', $product['video']);
            }
            ...
            $on_not_available_product = b2wl_get_setting('on_not_available_product');
            $on_not_available_variation = b2wl_get_setting('on_not_available_variation');
            $disable_add_new_variants = get_post_meta($product_id, '_b2w_disable_add_new_variants', true);
            $on_new_variation_appearance = $disable_add_new_variants ? "nothing" : b2wl_get_setting('on_new_variation_appearance');
            ...
            // Delete and then try to update _banggood_sku_props
            delete_post_meta($product_id, "_banggood_sku_props");
            if ($first_ava_var) {
                $banggood_sku_props_id_arr = array();
                foreach ($first_ava_var['attributes'] as $cur_var_attr) {
                    foreach ($product['sku_products']['attributes'] as $attr) {
            ...
            // update variations
            if (!b2wl_check_defined('B2WL_DO_NOT_IMPORT_VARIATIONS') && !$wc_product->is_type('external') && !empty($product['sku_products']['variations']) && count($product['sku_products']['variations']) > 1) {
                foreach ($product['sku_products']['variations'] as &$var) {
                    $var['image'] = (!isset($var['image']) || in_array(md5($var['image']), $product['skip_images'])) ? '' : $var['image'];
                }
            ...
            if ($first_ava_var) {
                if ($wc_product->get_status() == "trash") {
                    delete_post_meta($product_id, "_b2w_autoremove");
                    wp_untrash_post($product_id);
                }
                // product available >>>
                if ($wc_product->is_type('external') && $first_ava_var) {
                    $init_status = get_post_meta($product_id, '_b2w_init_product_status', true);
                    if ($wc_product->get_status() !== $init_status) {
                        $wc_product->set_status($init_status);
                    }
                    delete_post_meta($product_id, '_b2w_init_product_status');
                }
            } else {
                ...
                if ($on_not_available_product === 'trash') {
                    update_post_meta($product_id, "_b2w_autoremove", 1);
                    $wc_product->delete();
                    $deleted = true;
                } else if ($on_not_available_product === 'zero') {
                    $tmp_skip_meta = get_post_meta($product_id, "_b2w_skip_meta", true);
                    ...
                    if ($wc_product->is_type('external') && $cur_status !== 'draft') {
                        update_post_meta($product_id, '_b2w_init_product_status', $wc_product->get_status());
                        $wc_product->set_status('draft');
                    }

                    update_post_meta($product_id, "_b2w_skip_meta", $tmp_skip_meta);
                }
            }
            ...
            //B2WL_FIX_RELOAD_IMAGES - special flag (for update only), if product images is disapear, reload it.
            if (b2wl_check_defined('B2WL_FIX_RELOAD_IMAGES') && isset($product['images'])) {
                $old_thumb_id = get_post_thumbnail_id($product_id);
                if ($old_thumb_id) {
                    B2WL_Utils::delete_attachment($old_thumb_id, true);
                    delete_post_meta($product_id, '_thumbnail_id');
                }
                ...
                if ($old_image_gallery) {
                    ...
                    foreach ($image_ids as $image_id) {
                        B2WL_Utils::delete_attachment($image_id, true);
                    }
                    delete_post_meta($product_id, '_product_image_gallery');
                }
            ...
            if (isset($params['manual_update']) && $params['manual_update'] && b2wl_check_defined('B2WL_FIX_RELOAD_DESCRIPTION') && !b2wl_get_setting('not_import_description')) {
            ...
            if (empty($params['skip_last_update'])) {
                update_post_meta($product_id, '_b2w_last_update', time());
            }

            do_action('b2wl_after_upd_product', $product_id, $product, $params);

            delete_transient('wc_attribute_taxonomies');

            return apply_filters('b2wl_woocommerce_after_upd_product', $result, $product_id, $product, $params);
        }
        ...
        public function build_description($product_id, $product)
        {
            ...
            if (class_exists('DOMDocument')) {
                ...
                for ($i = $elements->length; --$i >= 0;) {
                    ...
                    } else if (!b2wl_get_setting('use_external_image_urls')) {
                        $tmp_title = isset($product['title']) && $product['title'] ? $product['title'] : "Product " . $product['id'];

                        // if have edited image, than user initial url
                        $clear_image_url = !empty($product['tmp_edit_images'][$img_id]) ? $e->getAttribute('src') : B2WL_Utils::clear_image_url($e->getAttribute('src'));
            ...
            if ($thumb_url && $thumb_url != 'empty' && (!get_post_thumbnail_id($product_id) || $update)
                ...
                try {
                    ...
                    if (is_wp_error($thumb_id)) {
                        b2wl_error_log("Can't download $thumb_url: " . print_r($thumb_id, true));
                    } else {
                        ...
                        if ($step === false || $step === 'images#' . md5($image_url)) {
                            ...
                            try {
                                ...
                                if (is_wp_error($new_image_gallery_id)) {
                                    b2wl_error_log("Can't download $image_url" . print_r($new_image_gallery_id, true));
                                } else {
        ...
        public function update_price(&$product, $variation, $rest_price = false)
        {
            ...
            if ($variation) {
                ...
                if (is_a($product, 'WC_Product')) {
                    update_post_meta($product->get_id(), '_banggood_regular_price', $regular_price);
                    update_post_meta($product->get_id(), '_banggood_price', $price);
                } else {
                    update_post_meta($product, '_banggood_regular_price', $regular_price);
                    update_post_meta($product, '_banggood_price', $price);
                }
                ...
                delete_post_meta($product, '_banggood_regular_price');
                delete_post_meta($product, '_banggood_price');
            }
        }

        private function set_attributes($product_id, $attributes)
        {
            if (defined('B2WL_IMPORT_EXTENDED_ATTRIBUTE')) {
                $extended_attribute = filter_var(B2WL_IMPORT_EXTENDED_ATTRIBUTE, FILTER_VALIDATE_BOOLEAN);
            } else {
                $extended_attribute = b2wl_get_setting('import_extended_attribute');
            }

            $attributes = apply_filters('b2wl_set_product_attributes', $attributes);
        ...
        private function add_variation($product_id, $product, $is_update = false, $params = array())
        {
            ...
            $localCurrency = strtoupper(b2wl_get_setting('local_currency'));
            ...
            if ($localCurrency) {
                ...
                $currency_conversion_factor = floatval(b2wl_get_setting('currency_conversion_factor'));
            }

            if (b2wl_check_defined('B2WL_FIX_RELOAD_VARIATIONS')) {
                delete_post_meta($product_id, '_b2w_original_variations_attributes');
            }

            $deleted_variations_attributes = get_post_meta($product_id, '_b2w_deleted_variations_attributes', true);
            ...
            $original_variations_attributes = get_post_meta($product_id, '_b2w_original_variations_attributes', true);
            ...
            $not_remove_variation_attr = b2wl_check_defined('B2WL_NOT_REMOVE_VARIATION_ATTR');
            foreach ($tmp_attributes as $key => $attr) {
                if (!intval($attr['is_variation']) || $not_remove_variation_attr) {
                    if (b2wl_check_defined('B2WL_FIX_VAR_PRODUCT_ATTRIBUTES')) {
            ...
            foreach ($variations['attributes'] as $key => $attr) {
                ...
                foreach ($attr['value'] as &$val) {
                    ...
                    if (!$has_variation && !b2wl_check_defined('B2WL_SKIP_REMOVED_VARIATIONS_CHECK')) {
                    ...
                    if (b2wl_get_setting('use_external_image_urls')) {
                    ...
                    $RELOAD_ATTR_IMAGES = b2wl_check_defined('B2WL_FIX_RELOAD_IMAGES') || b2wl_check_defined('B2WL_FIX_RELOAD_ATTR_IMAGES');
                    ...
                    if (($step === false || $step === 'variations#attributes') && $attr_image) {
                        ...
                        $attr_image = B2WL_Utils::clear_image_url($attr_image);
                        ...
                        if ($is_update && $RELOAD_ATTR_IMAGES) {
                            if (intval($old_attachment_id) > 0) {
                                B2WL_Utils::delete_attachment($old_attachment_id, true);
                            }
            ...
            if ($is_update && b2wl_check_defined('B2WL_FIX_RELOAD_VARIATIONS')) {
                $tmp_skip_meta = get_post_meta($product_id, "_b2w_skip_meta", true);
            ...
            foreach ($variations['variations'] as $variation) {
                ...
                $old_vid = $wpdb->get_row($wpdb->prepare("SELECT p.ID, p.post_status, pm_stock.meta_value as 'quantity', pm_regular.meta_value as 'regular_price', pm_sale.meta_value as 'price' FROM $wpdb->posts p " .
                    ...
                    "LEFT JOIN $wpdb->postmeta pm_regular ON (p.ID=pm_regular.post_id AND pm_regular.meta_key='_banggood_regular_price') " .
                    "LEFT JOIN $wpdb->postmeta pm_sale ON (p.ID=pm_sale.post_id AND pm_sale.meta_key='_banggood_price') " .
                ...
                if (!$old_vid && (!$is_update || $on_new_variation_appearance === 'add')) {
                    ...
                    if ($is_update && !$has_new_variants) {
                        ...
                        b2wl_info_log('Has new variant! Variant ID: ' . $variation['id']);
                    }
                    ...
                    $banggood_sku_props_id = $banggood_sku_props_id_arr ? implode(";", $banggood_sku_props_id_arr) : "";
                    if ($need_process && $banggood_sku_props_id) {
                        update_post_meta($variation_id, '_banggood_sku_props', $banggood_sku_props_id);
                    }
                    // <-- build _banggood_sku_props
                    ...
                    if ($is_update) {
                        if (!$is_price_changed) {
                            ...
                            if (!is_null($old_vid['price']) && !is_null($old_vid['regular_price'])) {
                                ...
                                if ($price !== $old_price || $regular_price !== $old_regular_price) {
                                    ...
                                    b2wl_info_log('Prices are changed! Variant ID: ' . $variation['id'] . ' Old Prices: ' . $old_price . ', ' . $old_regular_price . ', New Prices:' . $price . ', ' . $regular_price);
                        ...
                        b2wl_error_log("New stock quantity: " . print_r($variation['quantity'], true));
                        if (!$is_stock_changed) {
                            ...
                            if (!is_null($old_vid['quantity'])) {
                                ...
                                if ($quantity !== $old_quantity) {
                                    ...
                                    b2wl_info_log('Stock is changed! Variant ID: ' . $variation['id'] . ' Old Stock: ' . $old_quantity . ' New Stock: ' . $quantity);
                                }
                            }
                    ...
                    $banggood_sku_props_id = get_post_meta($variation_id, '_banggood_sku_props', true);
                    $banggood_sku_props_id_arr = $banggood_sku_props_id ? explode(";", $banggood_sku_props_id) : array();
                    ...
                    foreach ($used_variation_attributes as $attr_tax => $v) {
                        ...
                        if ($attr_value = get_post_meta($variation_id, $tmp_attr_name, true)) {
                            ...
                            foreach ($banggood_sku_props_id_arr as $var_attr_id) {
                    ...
                    // B2WL_FIX_RELOAD_IMAGES(or B2WL_FIX_RELOAD_ATTR_IMAGES) - special flag (for update only), if variation images is disapear, reload it.
                    if ($need_process && $is_update &&
                        (b2wl_check_defined('B2WL_FIX_RELOAD_IMAGES') || b2wl_check_defined('B2WL_FIX_RELOAD_ATTR_IMAGES')) &&
                ...
                if (isset($variation_id)) {
                    ...
                    if ($need_process) {
                        if (!empty($variation['warehouse'])) {
                            update_post_meta($variation_id, '_b2w_warehouse', $variation['warehouse']);
                        }
            ...
            if ($step === false || $step === 'variations#sync') {
                update_post_meta($product_id, '_b2w_original_variations_attributes', $original_variations_attributes);
                ...
                if (!$is_update && !empty($product['skip_attr'])) {
                    ...
                    update_post_meta($product_id, '_b2w_deleted_variations_attributes', $tmp_original_variations_attributes_tmp);
                }
                ...
                foreach ($old_variations as $old_variation) {
                    if ($on_not_available_variation === 'trash' || $old_variation['external_variation_id'] == 'delete') {
                        $GLOBALS['b2wl_autodelete_variaton_lock'] = true;
                        wp_delete_post($old_variation['id']);
                        unset($GLOBALS['b2wl_autodelete_variaton_lock']);
        ...
        {
            global $wpdb;
            $result = $wpdb->get_results("SELECT pm1.meta_value as ext_order_id, pm1.post_id as order_id FROM {$wpdb->postmeta} as pm1 LEFT JOIN {$wpdb->postmeta} as pm2 ON (pm2.meta_key = '_b2w_tracking_code' and pm1.post_id=pm2.post_id) WHERE pm1.meta_key = '_b2w_external_order_id' AND pm2.post_id is null AND pm1.meta_value <> ''");
            return $result;
        }
        ...
        {
            global $wpdb;
            $result = $wpdb->get_var("SELECT COUNT(*) as count FROM {$wpdb->postmeta} as pm1 LEFT JOIN {$wpdb->postmeta} as pm2 ON (pm2.meta_key = '_b2w_tracking_code' and pm1.post_id=pm2.post_id) WHERE pm1.meta_key = '_b2w_external_order_id' AND pm2.post_id is null AND pm1.meta_value <> ''");
            return $result;
        }
        ...
        public function save_tracking_code($order_id, $tracking_codes)
        {
            ...
            if (!$order) {
                $result = B2WL_ResultBuilder::buildError(_x('Didn`t find the Woocommerce order №', 'Error text', 'bng2woo-lite') . $order_id);
            } else {
                $curr_tracking_codes = get_post_meta($order_id, '_b2w_tracking_code');
                ...
                foreach ($tracking_codes as $code_value) {
                    ...
                    if (!empty($curr_tracking_codes) && is_array($curr_tracking_codes)) {
                        if (array_search($code_value, $curr_tracking_codes) === false) {
                            add_post_meta($order_id, '_b2w_tracking_code', $code_value);
                ...
                $order_external_id_array = get_post_meta($order->get_id(), '_b2w_external_order_id');

                $order_tracking_codes = get_post_meta($order->get_id(), '_b2w_tracking_code');
                ...
                if (is_array($order_external_id_array) && is_array($order_tracking_codes)
                    ...
                    //update order status automatically, when bng2woo-lite get all tracking codes only.

                    $tracking_code_order_status = b2wl_get_setting('tracking_code_order_status');
        ...
        public function get_sorted_products_ids($sort_type, $ids_count, $compare = false)
        {
            ...
            if ($compare) {
                ...
                if (!empty($operation) && !empty($value)) {
                    $query = $wpdb->prepare("SELECT p.ID FROM $wpdb->posts p INNER JOIN $wpdb->postmeta pm1 ON (pm1.post_id=p.ID AND pm1.meta_key='_b2w_external_id') LEFT JOIN $wpdb->postmeta pm2 ON (pm2.post_id=p.ID AND pm2.meta_key=%s) LEFT JOIN wp_postmeta pm3 ON (pm3.post_id=p.ID AND pm3.meta_key='_b2w_autoremove') WHERE p.post_type = 'product' AND pm1.meta_value != '' AND (p.post_status != 'trash' || NOT pm3.meta_key IS NULL) AND (pm2.meta_value is null OR pm2.meta_value $operation %s) ORDER BY pm2.meta_value LIMIT %d", $sort_type, $value, $ids_count);
                }
            } else {
        ...
        {
            global $wpdb;
            return $wpdb->get_var("SELECT count(DISTINCT post_id) from $wpdb->postmeta WHERE meta_key = '_b2w_external_id' and meta_value != '' and NOT meta_value IS NULL");
        }
        ...
        public function get_product_external_id($post_id)
        {
            ...
            if ($post) {
                if ($post->post_type === 'product') {
                    ...
                    $external_id = get_post_meta($post->post_parent, "_b2w_external_id", true);
                }
            }
            return $external_id;
        }
        ...
        public function get_product_by_post_id($post_id, $with_vars = true)
        {
            ...
            if ($external_id) {
                ...
                $product = array(
                    'id' => $external_id,
                    'post_id' => $post_id,
                    'url' => get_post_meta($post_id, "_b2w_original_product_url", true),
                    'affiliate_url' => get_post_meta($post_id, "_b2w_product_url", true),
                    'seller_url' => get_post_meta($post_id, "_b2w_seller_url", true),
                    'warehouse' => get_post_meta($post_id, "_b2w_warehouse", true),
                    'warehouse_list' => get_post_meta($post_id, "_b2w_warehouse_list", true),
                );
                ...
                $import_lang = get_post_meta($post_id, "_b2w_import_lang", true);
                $product['import_lang'] = $import_lang ? $import_lang : B2WL_BanggoodLocalizator::getInstance()->language;

                $price = get_post_meta($post_id, "_banggood_price", true);
                $regular_price = get_post_meta($post_id, "_banggood_regular_price", true);
                ...
                $original_product_url = get_post_meta($post_id, "_b2w_original_product_url", true);
                $product['original_product_url'] = $original_product_url ? $original_product_url : 'www.banggood.com/item//' . $product['id'] . '.html';

                $availability_meta = get_post_meta($post_id, "_b2w_availability", true);
                $product['availability'] = $availability_meta ? filter_var($availability_meta, FILTER_VALIDATE_BOOLEAN) : true;

                $b2wl_skip_meta = get_post_meta($post_id, "_b2w_skip_meta", true);

                $product['skip_vars'] = $b2wl_skip_meta && !empty($b2wl_skip_meta['skip_vars']) ? $b2wl_skip_meta['skip_vars'] : array();
                $product['skip_images'] = $b2wl_skip_meta && !empty($b2wl_skip_meta['skip_images']) ? $b2wl_skip_meta['skip_images'] : array();

                $shipping_meta = new B2WL_ProductShippingMeta($post_id);
                ...
                $product['disable_sync'] = get_post_meta($post_id, "_b2w_disable_sync", true);
                $product['disable_var_price_change'] = get_post_meta($post_id, "_b2w_disable_var_price_change", true);
                $product['disable_var_quantity_change'] = get_post_meta($post_id, "_b2w_disable_var_quantity_change", true);
                $product['disable_add_new_variants'] = get_post_meta($post_id, "_b2w_disable_add_new_variants", true);
                ...
                if ($with_vars) {
                    ...
                    if ($variations) {
                        foreach ($variations as $variation_id) {
                            ...
                            $price = get_post_meta($variation_id, "_banggood_price", true);
                            $regular_price = get_post_meta($variation_id, "_banggood_regular_price", true);
        ...
        {
            global $wpdb;
            return $wpdb->get_var($wpdb->prepare("SELECT post_id FROM $wpdb->postmeta WHERE meta_key='_b2w_external_id' AND meta_value='%s' LIMIT 1", $external_id));
        }

        public function get_product_id_by_import_id($import_id)
        {
            global $wpdb;
            return $wpdb->get_var($wpdb->prepare("SELECT post_id FROM $wpdb->postmeta WHERE meta_key='_b2w_import_id' AND meta_value='%s' LIMIT 1", $import_id));
        }
