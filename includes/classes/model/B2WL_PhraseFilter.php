...
/**
 * Description of B2WL_PhraseFilter
 *
 * @author ma_group
 */
class B2WL_PhraseFilter {
    ...
    public function save() {
        B2WL_PhraseFilter::$_instance_objects = null;
        B2WL_PhraseFilter::$_instance_array = null;
        
        $formula_list = B2WL_PhraseFilter::load_phrase_list(false);
        ...
        b2wl_set_setting('phrase_list', array_values($formula_list));
        return $this;
    }
    ...
    public static function apply_filter_to_text($text, $phrase_list=array()) {
        ...
        if(!$phrase_list){
            $phrase_list = B2WL_PhraseFilter::load_phrase_list(false);
        }
    ...
    public static function apply_filter_to_products() {
        ...
        $tmp_product_ids = $wpdb->get_results("SELECT p.ID, p.post_title, p.post_content from {$wpdb->posts} p LEFT JOIN {$wpdb->postmeta} as pm ON pm.post_id = p.ID WHERE (".implode(" OR ", $phrase_query_array).") AND pm.meta_key = '_b2w_external_id'", ARRAY_N);
        foreach ($tmp_product_ids as $row) {
            $new_title = B2WL_PhraseFilter::apply_filter_to_text($row[1], $phrase_list);
            $new_content = B2WL_PhraseFilter::apply_filter_to_text($row[2], $phrase_list);
    ...
    public static function deleteAll() {
        b2wl_del_setting('phrase_list');
    }

    public static function load_phrases() {
        return B2WL_PhraseFilter::load_phrase_list(true);
    }

    private static function load_phrase_list($asObject = true) {
        $result = array();

        if ($asObject && B2WL_PhraseFilter::$_instance_objects) {
            return B2WL_PhraseFilter::$_instance_objects;
        } else if (!$asObject && B2WL_PhraseFilter::$_instance_array) {
            return B2WL_PhraseFilter::$_instance_array;
        }
        
        $formula_list = b2wl_get_setting('phrase_list');
        $formula_list = $formula_list && is_array($formula_list) ? $formula_list : array();

        if ($asObject) {
            foreach ($formula_list as $formula) {
                $fo = new B2WL_PhraseFilter();
                ...
                $result[] = $fo;
                B2WL_PhraseFilter::$_instance_objects = $result;
            }
        } else {
            $result = $formula_list;
            B2WL_PhraseFilter::$_instance_array = $result;
        }
...
if (!function_exists('b2wl_phrase_apply_filter_to_text')) {

    function b2wl_phrase_apply_filter_to_text($text) {
        return B2WL_PhraseFilter::apply_filter_to_text($text);
    }
