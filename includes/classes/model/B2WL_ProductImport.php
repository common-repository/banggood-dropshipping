...
/**
 * Description of B2WL_ProductImport
 *
 * @author andrey
 */
if (!class_exists('B2WL_ProductImport')) {

    class B2WL_ProductImport {
        ...
        public function del_product($product_id, $processing = false) {
            ...
            if ($to_del) {
                foreach ($to_del as $did) {
                    b2wl_delete_transient('b2wl_'.($processing?'processing_':'').'product#' . strval($did));
                }
            }
        }
        ...
        public function save_product($product_id, $product, $processing = false) {
            b2wl_set_transient('b2wl_'.($processing?'processing_':'').'product#' . strval($product_id), $product);
        }

        public function get_product($product_id, $processing = false) {
            $product = b2wl_get_transient('b2wl_'.($processing?'processing_':'').'product#' . strval($product_id));
            if ($product) {
                if(!isset($product['import_id']) || !$product['import_id']){
                    // 1.7.0 for compatibility with older versions
                    $product['import_id'] = $product['id'];
                ...
                if (!isset($product['product_type'])) {
                    $product['product_type'] = b2wl_get_setting('default_product_type');
                }
                if (!isset($product['product_status'])) {
                    $product['product_status'] = b2wl_get_setting('default_product_status');
                }
        ...
        public function get_product_id_list($limit=null, $ofset=null) {
            ...
            if (b2wl_check_defined('B2WL_SAVE_TRANSIENT_AS_OPTION')) {
                $results = $wpdb->get_results("select option_name from {$wpdb->options} where option_name like 'b2wl_product#%' order by option_id $limitStr", ARRAY_A);
            } else {
                $results = $wpdb->get_results("select option_name from {$wpdb->options} where option_name like '_transient_b2wl_product#%' order by option_id $limitStr", ARRAY_A);
            }
            $ids = array();
        ...
        public function get_products_count() {
            global $wpdb;
            if (b2wl_check_defined('B2WL_SAVE_TRANSIENT_AS_OPTION')) {
                return $wpdb->get_var("select count(option_id) from {$wpdb->options} where option_name like 'b2wl_product#%'");
            } else {
                return $wpdb->get_var("select count(option_id) from {$wpdb->options} where option_name like '_transient_b2wl_product#%'");
            }
        }
        ...
        public function sort_list() {
            return array('id-asc'=>__('Sort by External Id', 'bng2woo-lite'),'title-asc'=>__('Sort by Product title', 'bng2woo-lite'),'date_add-asc'=>__('Sort by Date add (old first)', 'bng2woo-lite'),'date_add-desc'=>__('Sort by Date add (new first)', 'bng2woo-lite'));
        }
