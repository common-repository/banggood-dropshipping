...
/**
 * Description of B2WL_Override
 *
 * @author Andrey
 */

if (!class_exists('B2WL_Override')) {
    class B2WL_Override {
        ...
        public function has_override($product_id){
            ...
            if($product_id){
                return !!$wpdb->get_var("SELECT 1 FROM $wpdb->options WHERE option_name like '%b2wl_product%' AND option_value like '%\"override_product_id\";i:".intval($product_id)."%'");
            }else{
                return false;
            }
        }
        ...
        public function override($product_id, $external_id, $override_images=false, $override_title_description=false, $order_variations=array()){
            ...
            if($this->has_override($product_id)){
                $result = array(
                    "state"=>"error", 
                    "message"=>__("You've already selected to override this product. Check your import list and confirm the override to continue.", "bng2woo-lite")
                );
            }else{
        ...
        public function override_message($product_id, $product_title){
            $msg = sprintf(__('This product will override <a href="%s">%s</a> Click "Override" to proceed.', 'bng2woo-lite'), 
                admin_url('post.php?post='.intval($product_id).'&action=edit'), 
                $product_title
            );
            $btn = '<button class="btn btn-default cancel-override" type="button">'.__('Cancel Override', 'bng2woo-lite').'</button>';
