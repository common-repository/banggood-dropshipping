...
/**
 * Description of B2WL_PriceFormula
 *
 * @author Andrey
 */
class B2WL_PriceFormula
{
    ...
    public static function pricing_rules_types()
    {
        return array(
            'sale_price_and_discount' => array(
                ...
                'name' => __('Use sale price and discount', 'bng2woo-lite'),
                'description' => __('Bng2Woo Lite gets your <strong>sale price</strong> by applying a pricing formula to the Banggood sale price. Your <strong>regular price</strong> is calculated using the <i>Regular Price</i> part of the formula.<br/> If you disable that part, the <strong>regular price</strong> is formed from your calculated sale price increased by the original Banggood discount.<br/> To learn more check out this article.', 'bng2woo-lite'),
            ),
            'sale_price_as_base' => array(
                'value' => 'sale_price_as_base',
                'name' => __('Use sale price as base', 'bng2woo-lite'),
                'description' => __('Your pricing formula is applied to the Banggood sale price. To learn more check out this article.', 'bng2woo-lite'),
            ),
            'regular_price_as_base' => array(
                'value' => 'regular_price_as_base',
                'name' => __('Use regular price as base', 'bng2woo-lite'),
                'description' => __('Your pricing formula is applied to the Banggood regular price. To learn more check out this article.', 'bng2woo-lite'),
            ),
        );
    }
    ...
    public function load($id = false)
    {
        ...
        if ($load_id) {
            $formula_list = B2WL_PriceFormula::load_formulas_list(false);
    ...
    public function save()
    {
        ...
        b2wl_set_setting('formula_list', array_values($formula_list));
        return $this;
    }
    ...
    {
        $pricing_rules_type = b2wl_get_setting('pricing_rules_type');

        $apply_price_rules_after_shipping_cost = b2wl_get_setting('apply_price_rules_after_shipping_cost');
        $shipping_cost = b2wl_get_setting('add_shipping_to_price') && !empty($product['shipping_cost']) ? round($product['shipping_cost'], $round) : 0;

        if (!isset($product['calc_price'])) {
            $product['calc_price'] = array();
        }

        if (!isset($product['calc_regular_price'])) {
            $product['calc_regular_price'] = array();
        }

        if (b2wl_check_defined('B2WL_USE_SEPARATE_FORMULA')) {
            $formula = B2WL_PriceFormula::get_formula_by_product($product, "price");
            $formula_regular = B2WL_PriceFormula::get_formula_by_product($product, "regular_price");
        } else {
            $price_type = $pricing_rules_type == 'regular_price_as_base' ? "regular_price" : "price";
            $formula = B2WL_PriceFormula::get_formula_by_product($product, $price_type);
            $formula_regular = B2WL_PriceFormula::get_formula_by_product($product, $price_type);
        }

        $product_price = B2WL_PriceFormula::normalize_product_price($product);

        if ($formula && $formula_regular && $product_price['price']) {
            $use_compared_price_markup = b2wl_get_setting('use_compared_price_markup');
            $price_cents = b2wl_get_setting('price_cents');
            $price_compared_cents = b2wl_get_setting('price_compared_cents');
            ...
            if (isset($product['sku_products']['variations']) && $product['sku_products']['variations']) {
                foreach ($product['sku_products']['variations'] as &$var) {
                    if (!isset($var['calc_price'])) {
                        $var['calc_price'] = 0;
                    }

                    if (!isset($var['calc_regular_price'])) {
                        $var['calc_regular_price'] = 0;
                    }

                    if (b2wl_check_defined('B2WL_USE_SEPARATE_FORMULA')) {
                        $formula = B2WL_PriceFormula::get_formula_by_product($var, false, "price");
                        $formula_regular = B2WL_PriceFormula::get_formula_by_product($var, false, "regular_price");
                    } else {
                        $price_type = $pricing_rules_type == 'regular_price_as_base' ? "regular_price" : "price";
                        $formula = B2WL_PriceFormula::get_formula_by_product($var, false, $price_type);
                        $formula_regular = B2WL_PriceFormula::get_formula_by_product($var, false, $price_type);
                    }
    ...
    {
        $formula = b2wl_get_setting('default_formula');
        return new B2WL_PriceFormula($formula && is_array($formula) ? $formula : array('value' => 1, 'sign' => '*', 'compared_value' => 1, 'compared_sign' => '*'));
    }

    public static function set_default_formula($formula)
    {
        b2wl_set_setting('default_formula', get_object_vars($formula));
    }

    public static function get_default_formulas()
    {
        $f1 = new B2WL_PriceFormula();
        $f1->id = 1;
        $f1->min_price = 0;
        $f1->max_price = 10;
        $f1->sign = "*";
    ...
    {
        $res_formula = false;
        $use_extended_price_markup = b2wl_get_setting('use_extended_price_markup');
        ...
        if ($use_extended_price_markup) {
            ...
            if ($product_price) {
                $formula_list = B2WL_PriceFormula::load_formulas_list();
                foreach ($formula_list as $formula) {
                    $check = true;
                ...
                b2wl_error_log("can't find normalize_product_price for " . $product['id']);
            }
        }

        return $res_formula ? $res_formula : B2WL_PriceFormula::get_default_formula();
    }
}
