...
var b2wl_reload_page_after_ajax = false;
...
function b2wl_get_product_proc(params, callback) {
    if (typeof b2wl_get_product === "function") {
        ...
        callback('error', false, 'Please install and activate the bng2woo-lite chrome extension in your browser: <a href="' + b2wl_wc_pl_script.chrome_url + '">Get Chrome Extension</a>');
    }
}
...
(function ($, window, document, undefined) {
    $(function () {
        ...
        $(document).on("click", ".b2wl-product-info", function () {
            var id = $(this).attr('id').split('-')[1];
            $.b2wl_show(id);
            return false;
        });

        $.b2wl_show = function (id) {
            $('<div id="b2wl-dialog' + id + '"></div>').dialog({
                dialogClass: 'b2wl-dialog',
                width: "400px",
                modal: true,
                title: "Banggood Info (ID: " + id + ")",
                open: function () {
                    $('#b2wl-dialog' + id).html(b2wl_wc_pl_script.lang.please_wait_data_loads);
                    var data = { 'action': 'b2wl_product_info', 'id': id };
                    ...
                    $.post(ajaxurl, data, function (response) {
                        ...
                        if (json.state === 'error') {
                            ...
                            $('#b2wl-dialog' + json.data.id).html(json.data.content.join('<br/>'));
                        }
        ...
        $("#doaction, #doaction2").click(function (e) {
            ...
            if (!b2wl_update_action_lock && ('b2wl_product_update_manual' === check_action)) {
                e.preventDefault();
                ...
                var cnt = 0;
                var total_ids = 0;
                var data = { 'action': ('b2wl_product_update_manual' === check_action) ? 'b2wl_sync_products' : '', 'ids': [] }
                $('input:checkbox[name="post[]"]:checked').each(function () {
                    ...
                    if (cnt === update_per_request) {
                        ...
                        cnt = 0;
                        data = { 'action': ('b2wl_product_update_manual' === check_action) ? 'b2wl_sync_products' : '', 'ids': [] };
                    }
                });
                ...
                if (total_ids > 0) {
                    b2wl_show_block($('.wp-list-table.posts'), b2wl_sprintf(b2wl_wc_pl_script.lang.process_update_d_of_d, 0, total_ids));

                    b2wl_sprintf(b2wl_wc_pl_script.lang.process_update_d_of_d, 0, total_ids)
                    var on_load = function (response_state, state) {
                        if ((state.update_cnt + state.update_error_cnt) === state.num_to_update) {
                            b2wl_update_block($(".wp-list-table.posts"), b2wl_sprintf(b2wl_wc_pl_script.lang.complete_result_updated_d_erros_d, state.update_cnt, state.update_error_cnt));
                            b2wl_update_action_lock = false;
                            location.reload();
                        } else {
                            b2wl_update_block($(".wp-list-table.posts"), b2wl_sprintf(b2wl_wc_pl_script.lang.process_update_d_of_d_erros_d, state.update_cnt, state.num_to_update, state.update_error_cnt));
                        }
                    };
...
function b2wl_show_block(element, message) {
...
function b2wl_update_block(element, message) {
    jQuery(element).find('.blockUI.blockMsg').html(message);
}

function b2wl_hide_block(element) {
